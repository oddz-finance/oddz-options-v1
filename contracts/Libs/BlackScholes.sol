...
library BlackScholes {
    ...
    function _blackScholesCalculation(
        ...
        return
            _priceCalculation(isCallOption, strikePrice, currentPrice, interestRate, dividendYield, expiration, d1, d2);
    }
        ...
        if (isCallOption) {
            return
                ABDKMath64x64.sub(
                    ABDKMath64x64.mul(
                        dividendYieldFactor,
                        ABDKMath64x64.mul(currentPrice, _normalCummulativeDistribution(d1))
                    ),
                    ABDKMath64x64.mul(
                        interestRateFactor,
                        ABDKMath64x64.mul(strikePrice, _normalCummulativeDistribution(d2))
                    )
                );
        } else {
            return
                ABDKMath64x64.sub(
                    ABDKMath64x64.mul(
                        interestRateFactor,
                        ABDKMath64x64.mul(strikePrice, _normalCummulativeDistribution(ABDKMath64x64.neg(d2)))
                    ),
                    ABDKMath64x64.mul(
                        dividendYieldFactor,
                        ABDKMath64x64.mul(currentPrice, _normalCummulativeDistribution(ABDKMath64x64.neg(d1)))
                    )
                );
        }
    }
        ...
        int128 f =
            ABDKMath64x64.mul(
                ...
                ABDKMath64x64.add(
                    ...
                    ABDKMath64x64.mul(
                        t,
                        ABDKMath64x64.add(-0x17401c57014c38f14, ABDKMath64x64.mul(t, 0x10fb844255a12d72e))
                    )
                )
            );
        int128 f2 =
            ABDKMath64x64.add(0x413c831bb169f874, ABDKMath64x64.mul(t, ABDKMath64x64.add(-0x48d4c730f051a5fe, f)));
        return
            ABDKMath64x64.sub(
                0x10000000000000000,
                ABDKMath64x64.mul(
                    t,
                    ABDKMath64x64.mul(f2, ABDKMath64x64.exp(ABDKMath64x64.mul(ABDKMath64x64.neg(z), z)))
                )
            );
    }
