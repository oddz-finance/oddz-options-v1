...
export function shouldBehaveLikeOddzToken(): void {
  ...
  it("should successfully transfer full balance", async function () {
    const oddzToken = await this.oddzToken.connect(this.signers.admin);
    await oddzToken.transfer(this.accounts.admin1, TotalSupply);
    expect(await oddzToken.balanceOf(this.accounts.admin1)).to.equal(TotalSupply);
  });
  ...
  it("should transfer token by allowed spender", async function () {
    ...
    await oddzToken
      ...
      .transferFrom(this.accounts.admin, this.delegatorAccounts.admin, amount);
  ...
  it("Should be able to get back accidentally sent tokens", async function () {
    ...
    // Rescue user funds
    await expect(
      oddzToken.connect(this.signers.admin).rescueTokens(oddzToken.address, this.accounts.admin1, 100),
    ).to.emit(oddzToken, "RescueExcessTokens");
    expect(await oddzToken.balanceOf(this.accounts.admin1)).to.equal(100);
  });
  ...
  */
  ...
  //   await expect(oddzToken.permit(this.accounts.admin, this.accounts.admin1, amount, deadline, v, r, s)).to.emit(
  //     oddzToken,
  //     "Approval",
  //   );
  //   expect(await oddzToken.allowance(this.accounts.admin, this.accounts.admin1)).to.equal(amount);
