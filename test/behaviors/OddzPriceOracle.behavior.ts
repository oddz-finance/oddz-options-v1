...
export function shouldBehaveLikeOddzPriceOracle(): void {
  it("Should be able to successfully set the aggregator", async function () {
    const oracle = await this.oddzPriceOracle.connect(this.signers.admin);
    await expect(oracle.setAggregator(AssetIds.ETH, AssetIds.USDT, this.MockAggregator.address)).to.emit(
      oracle,
      "SetAggregator",
    );
  });
